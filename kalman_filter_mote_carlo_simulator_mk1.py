...
class Model:
    ...
    def generate_crlb(self):
        ...
        derivate_bearing_to_stateX = np.zeros((self.steps, 4))
        j = 0  # 因为一般估计的是初始时间下的状态

        for i in range(self.steps):
            xt_i, yt_i, _, _ = self.target_states[i]
            xo_i, yo_i = self.sensor_trajectory[i]
            d_Bi_to_Xtxj = (yt_i - yo_i) / ((xt_i - xo_i) ** 2 + (yt_i - yo_i) ** 2)

            d_Bi_to_Xtyj = -(xt_i - xo_i) / ((xt_i - xo_i) ** 2 + (yt_i - yo_i) ** 2)

            d_Bi_to_Xtvx = (self.times[i] - self.times[j]) * d_Bi_to_Xtxj
            d_Bi_to_Xtvy = (self.times[i] - self.times[j]) * d_Bi_to_Xtyj

            derivate_bearing_to_stateX[i, :] = [d_Bi_to_Xtxj, d_Bi_to_Xtyj, d_Bi_to_Xtvx, d_Bi_to_Xtvy]

        fim = derivate_bearing_to_stateX.T @ derivate_bearing_to_stateX / self.R
        fim_inv = np.linalg.inv(fim)
        crlb = np.diag(fim_inv)
        self.crlb[i][0] = crlb[0]
        self.crlb[i][1] = crlb[1]
        self.crlb[i][2] = crlb[2]
        self.crlb[i][3] = crlb[3]
...
class Runner:
    ...
    def generate_monte_carlo_data(self, num):
        ...
        # 生成轨迹（仅需一次）
        self.model.generate_sensor_trajectory_8()
    ...
    def run_monte_carlo(self, num=None):
        ...
        runner_result = {'name': self.method_name,
                         ...
                         'time': np.arange(0, self.model.steps * self.model.sample_time + self.model.sample_time,
...
class Visulation:
    ...
    def plot_figure(self):
        ...
        plt.figure()
        crlbx = self.plot_result[0]['crlb']
        plt.plot(times_range, crlbx)
        plt.xlabel('时间 (s)')
        plt.ylabel('x位置估计crlb (m)')
        plt.title('x位置估计clrb')

...
class Algorithms:
    ...
    def run_ckf(self, x0, P0, Q):
        ...
        return {'states': estimated_states,
                'covs': estimated_covs,
                'pos_rmse': pos_rmse,
                'vel_rmse': vel_rmse, }
    ...
    def run_frckf(self, x0, P0, Q, backward_start_step):
        ...
        return {'states': optimized_forward_states,
...
class Visulation:
    ...
    def plot_figure(self):
        ...
        for i in range(num_of_methods_used - 1):
            estimation = self.plot_result[i]['estimation']
            name = self.plot_result[i]['name']
            plt.plot(estimation[:, 0], estimation[:, 1], 'r--', label=f"{name}算法{num}次平均估计轨迹")
        ...
        for i in range(num_of_methods_used - 1):
            ...
            plt.plot(times_range, pos_rmse, label=f"{name}算法{num}次仿真平均位置误差")
        ...
        for i in range(num_of_methods_used - 1):
            ...
            plt.plot(times_range, pos_rmse, label=f"{name}算法{num}次仿真平均速度误差")

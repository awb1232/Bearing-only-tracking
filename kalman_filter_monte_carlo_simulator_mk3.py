...
class Ship:
    def __init__(self, state, mode, acceleration, omega, maneuver=None):
        ...
        self.course = np.deg2rad( np.arctan2(state[2], state[3]) )
        self.speed = np.sqrt(state[2]**2 + state[3]**2)
...
class Model:
    ...
    def generate_sensor_trajectory_8(self):
        ...
        for k in range(self.steps + 1):
            ...
            self.sensor_trajectory[k, 0] = 500 * np.sin(t / 20)
            self.sensor_trajectory[k, 1] = 250 * np.sin(t / 10)
    ...
    def generate_sensor_trajectory_s(self, initial_heading_deg=45, amplitude=250, wavelength=500,):
        """
        生成初始航向为45°的S型轨迹

        参数:
        initial_heading_deg: 初始航向角，单位为度，默认为45度
        amplitude: S型曲线的振幅，决定左右偏移的最大距离
        wavelength: S型曲线的波长，决定一个完整S型的长度
        speed: 运动速度，默认为5 m/s
        ...
        for k in range(self.steps + 1):
            t = k * self.sample_time
            distance = self.Sensor.speed * t  # 沿主航向的距离

            # S型横向偏移（使用正弦函数创建S形）
            lateral_offset = amplitude * np.sin(2 * np.pi * distance / wavelength)

            # 计算位置：初始位置 + 沿航向的位移 + 横向偏移
            self.sensor_trajectory[k, 0] = start_x + dir_x * distance + perp_x * lateral_offset
            self.sensor_trajectory[k, 1] = start_y + dir_y * distance + perp_y * lateral_offset
...
if __name__ == '__main__':
    ...
    noise_mean = 0
    noise_std = 0.2
    Sensor = Ship(np.array([0,0,0,2]), mode='bdcv', acceleration=1, omega=1, maneuver='o')
    Target = Ship(np.array([5000, 5000, 0, -5]), mode='xyvv', acceleration=1, omega=1)
